...
class PC28APIClient:
    ...
    async def get_historical_data(self, date=None, limit=30):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(historical_url, params=params, timeout=10) as response:
                    if response.status == 200:
                        data = await response.json()
        ...
        try:
            ...
            for draw_record in retdata:
                try:
                    ...
                    if len(numbers) != 3:
                        logger.warning(f"期号 {draw_record.get('long_issue')} 开奖号码不完整: {numbers}")
                        continue
                    ...
                    current_row = {
                        ...
                        'timestamp': draw_record.get('kjtime', datetime.now().isoformat()),
                        'a': numbers_int[0],
                        'b': numbers_int[1],
                        'c': numbers_int[2],
                        'sum': total_sum,
                    ...
                    check_job_config = bigquery.QueryJobConfig(
                        query_parameters=[
                            bigquery.ScalarQueryParameter("issue", "STRING", current_row['issue'])
                        ]
                    )

                    result = bq_client.query(check_query, job_config=check_job_config).result()
                    ...
                    logger.error(f"处理历史记录失败 {draw_record.get('long_issue', 'unknown')}: {e}")
                    continue
    ...
    def _insert_draw_data(self, row: dict):
        ...
        job_config = bigquery.QueryJobConfig(
            query_parameters=[
                ...
                bigquery.ScalarQueryParameter("api_codeid", "INTEGER", row['api_codeid']),
                bigquery.ScalarQueryParameter("api_message", "STRING", row['api_message']),
                bigquery.ScalarQueryParameter("api_curtime", "INTEGER", row['api_curtime']),
                bigquery.ScalarQueryParameter("kjtime_raw", "STRING", row['kjtime_raw']),
                ...
                bigquery.ScalarQueryParameter("award_time", "INTEGER", row['award_time']),
                bigquery.ScalarQueryParameter("raw_api_response", "STRING", row['raw_api_response']),
            ]
        )
    ...
    def _insert_next_period_info(self, next_info: dict):
        ...
        job_config = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("current_issue", "STRING", next_info['current_issue']),
                bigquery.ScalarQueryParameter("next_issue", "STRING", next_info['next_issue']),
                bigquery.ScalarQueryParameter("next_time", "STRING", next_info['next_time']),
                bigquery.ScalarQueryParameter("award_time", "INTEGER", next_info.get('award_time')),
                bigquery.ScalarQueryParameter("created_at", "TIMESTAMP", next_info['created_at']),
            ]
        )
    ...
    try:
        ...
        if draw_data:
            ...
            return jsonify({
                ...
                'api_message': draw_data.get('message'),
                'saved': success,
                'fields_captured': len([k for k in draw_data.keys() if draw_data[k] is not None])
            })
        else:
...
async def test_api():
    ...
    try:
        ...
        if draw_data:
            # 分析API字段利用情况
            expected_fields = [
                'award_time', 'codeid', 'curent', 'kjtime', 'long_issue',
                'message', 'next', 'next_issue', 'next_time', 'number',
                'retdata', 'time'
            ]
...
async def backfill_historical():
    ...
    try:
        ...
        if historical_data:
            success, saved_count = api_client.save_historical_data_to_bigquery(historical_data)
            ...
            return jsonify({
                ...
                'skipped_count': len(historical_data.get('retdata', [])) - saved_count,
                'api_message': historical_data.get('message')
            })
        else:
            return jsonify({'status': 'no_data'}), 404
