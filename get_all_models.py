...
class AllModelsRetriever:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.models_endpoint, headers=headers, timeout=30
                ) as response:
                    ...
                    if response.status == 200:
                        ...
                        if "data" in data:
                            ...
                            with open(
                                "all_available_models.json", "w", encoding="utf-8"
                            ) as f:
                                json.dump(
                                    complete_models_data,
                                    f,
                            ...
                            print("📄 完整列表已保存: all_available_models.json")
    ...
    def analyze_models(self, models):
        ...
        top_providers = sorted(
            analysis["providers"].items(), key=lambda x: x[1], reverse=True
        )[:5]
    ...
    async def test_premium_models(self, analysis):
        ...
        chat_models = [
            ...
            if "chat" in m.get("type", "").lower() or m.get("type") == "chat-completion"
        ]
...
async def main():
    ...
    if analysis:
        ...
        print("📄 可用模型配置已保存: usable_models_config.json")
    else:
        print("❌ 模型获取失败")
