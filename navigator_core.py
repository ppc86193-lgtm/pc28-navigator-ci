        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.config.upstream_api_base}/259", params=params
                ) as response:
...
class NavigatorCore:
    ...
    async def emergency_diagnosis(self) -> Dict[str, Any]:
        ...
        diagnosis_result = {
            ...
            "recommendations": self._generate_recommendations(
                upstream_health, signal_diagnosis
            ),
        }
    ...
    def _generate_recommendations(
        self, upstream: Dict[str, Any], signal: Dict[str, Any]
    ) -> List[str]:
        """生成修复建议"""
        recommendations = []
    ...
    async def run_emergency_mode(self):
        ...
        try:
            ...
            # 保存诊断报告
            report_file = (
                f"emergency_diagnosis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
