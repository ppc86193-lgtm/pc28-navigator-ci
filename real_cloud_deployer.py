    ...
    async def check_gcloud_auth(self):
        ...
        try:
            ...
            if "ACTIVE" in auth_result.stdout:
                print("   ✅ Google Cloud认证已激活")
                return True
            else:
                print("   ❌ 需要Google Cloud认证")
                return False
...
"""
    ...
    async def create_vertex_ai_training_job(self):
        ...
        training_job_name = (
            f"pc28-training-job-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )
        ...
        try:
            ...
            print("   📋 训练作业配置: training_job_config.json")
    ...
    async def execute_real_deployment(self):
        ...
        if docker_result["status"] not in ["PUSHED"]:
            print("❌ Docker镜像处理失败，跳过后续部署")
            # 继续其他配置

        # 4. 部署Cloud Run服务
        if docker_result.get("status") == "PUSHED":
            cloudrun_result = await self.deploy_cloud_run_service(
                docker_result["image_name"]
            )
        else:
        ...
        # 保存真实部署报告
        report_file = (
            f"real_deployment_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
