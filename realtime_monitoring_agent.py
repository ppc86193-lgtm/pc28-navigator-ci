    ...
    async def check_new_predictions(self):
        ...
        try:
            ...
            if results:
                ...
                # 检查是否是新预测
                if self.last_prediction_time is None or prediction_time > self.last_prediction_time:
                    self.last_prediction_time = prediction_time
                    
                    # 发送新预测通知
                    await self.send_new_prediction_alert(latest_prediction)
                    return {"new_prediction": True, "prediction": dict(latest_prediction)}
                else:
                    print(f"   📊 无新预测")
    ...
    async def check_kpi_changes(self):
        ...
        try:
            ...
            if results:
                ...
                # 检查是否需要发送KPI报告（每小时一次）
                now = datetime.now()
                if self.last_kpi_check is None or (now - self.last_kpi_check).seconds > 3600:
