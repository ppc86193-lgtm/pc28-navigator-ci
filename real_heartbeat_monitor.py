...
class PC28RealHeartbeatMonitor:
    ...
    async def send_heartbeat(self):
        ...
        try:
            ...
            print(f"💓 心跳发送: 期号{latest_issue}, 错误{self.error_count}, {datetime.now().strftime('%H:%M:%S')}")
            return True
...
🎪 PC28为您实时服务！"""
                    ...
                    if push_result.get("success"):
                        print(f"🎲 推送开奖: 期号{latest.issue}, 结果{latest.sum} (消息ID: {push_result.get('message_id')})")
                    else:
                        print(f"❌ 推送失败: {push_result.get('error')}")
                        self.error_count += 1
                    
                    return {"new_draw": True, "issue": latest.issue, "push_result": push_result}
                else:
    ...
    async def monitoring_loop(self):
        ...
        try:
            while True:
                ...
                # 记录循环状态
                print(f"🔄 循环#{loop_count} {current_time.strftime('%H:%M:%S')} - 心跳: {'✅' if loop_count % 3 == 1 else '⏸️'}, 开奖: {'🎲' if draw_result.get('new_draw') else '📊'}")
