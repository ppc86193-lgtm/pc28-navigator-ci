...
class RealtimeMonitorAgent:
        ...
        try:
            ...
            monitor_report = {
                ...
                "data_freshness": {
                    ...
                    "latest_draw": draws_status.latest_draw.isoformat() if draws_status and draws_status.latest_draw else None,
                    "minutes_since_last": int(draws_status.minutes_since_last) if draws_status else None,
                    "freshness_status": "FRESH" if draws_status and draws_status.minutes_since_last < 10 else "STALE"
                },
                "prediction_status": {
                    "prediction_count": int(pred_status.prediction_count) if pred_status else 0,
                    "avg_p_star": float(pred_status.avg_p_star) if pred_status and pred_status.avg_p_star else 0,
                    "p_star_range": [
                        float(pred_status.min_p_star) if pred_status and pred_status.min_p_star else 0,
                        float(pred_status.max_p_star) if pred_status and pred_status.max_p_star else 0
                    ],
                    "latest_prediction": pred_status.latest_prediction.isoformat() if pred_status and pred_status.latest_prediction else None
                },
                "signal_status": {
                    "total_candidates": int(cand_status.total_candidates) if cand_status else 0,
                    "valid_candidates": int(cand_status.valid_candidates) if cand_status else 0,
                    ...
                    "signal_generation_status": "WORKING" if cand_status and cand_status.valid_candidates > 0 else "STOPPED"
                }
            }
    ...
    def run_task(self):
        ...
        # 保存真实回执
        receipt_file = f"monitor_receipt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        if "error" not in monitor_result:
            ...
            print(f"\n📊 监控结果:")
            print(f"   📈 开奖数据: {data_fresh['today_draws']}期, 新鲜度: {data_fresh['freshness_status']}")
            print(f"   🎯 预测数据: {pred_status['prediction_count']}条, 平均p_star: {pred_status['avg_p_star']:.3f}")
            print(f"   🚨 信号状态: {signal_status['valid_candidates']}个有效信号, 状态: {signal_status['signal_generation_status']}")
