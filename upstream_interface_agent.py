...
class PC28UpstreamInterfaceAgent:
    ...
    async def check_upstream_interfaces(self):
        ...
        for interface in upstream_interfaces:
            ...
            print(f"      下期时间: {'✅ 提供' if interface['provides_next_time'] else '❌ 不提供'}")
    ...
    async def get_next_draw_time(self):
        ...
        # 基于当前开奖时间计算下期时间
        current_time = datetime.strptime(self.current_result["draw_time"], "%Y-%m-%d %H:%M:%S")
...
系统将在下期开奖前1分钟推送预测！"""
        ...
        if result.get("success"):
            print(f"   ✅ 下期开奖时间通知发送成功")
        else:
            print(f"   ❌ 下期开奖时间通知发送失败")
    ...
    async def execute_interface_check(self):
        ...
        print("👑 监督者: 项目总指挥大人'小财神'")
        print("🎯 任务: 检查上游接口，获取下期开奖时间")
        print(f"📊 当期开奖: {self.current_result['issue']} = {self.current_result['sum']}")
        print()
        ...
        # 4. 发送下期开奖通知
        notification_result = await self.send_next_draw_notification(next_draw_info, pattern_analysis)
        ...
        # 保存检查报告
        report_file = f"upstream_interface_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(check_report, f, indent=2, ensure_ascii=False, default=str)
        
        print(f"\n🏆 上游接口Agent任务完成！")
        print(f"   检查时长: {check_duration:.1f}秒")
        print(f"   活跃接口: {interface_status['active_interfaces']}/{interface_status['total_interfaces']}")
        print(f"   下期时间: {next_draw_info['next_draw_time']}")
