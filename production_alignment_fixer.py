...
class ProductionAlignmentFixer:
        ...
        try:
            ...
            if results:
                ...
                return {
                    ...
                    "coverage_at_78": (
                        row.signals_at_78 / row.total_periods
                        if row.total_periods > 0
                        else 0
                    ),
                    "coverage_at_60": (
                        row.signals_at_60 / row.total_periods
                        if row.total_periods > 0
                        else 0
                    ),
                    "coverage_at_50": (
                        row.signals_at_50 / row.total_periods
                        if row.total_periods > 0
                        else 0
                    ),
    ...
    def _create_alignment_plan(self, production: Dict[str, Any]) -> Dict[str, Any]:
        ...
        plan = {
            ...
            "phase_3_monitoring": {
                "description": "建立持续监控",
                "actions": [
                    {"action": "部署Navigator实时监控", "coverage": "全链路监控"},
                    {"action": "建立TDR自动处置", "scope": "故障自动恢复"},
                ],
                "timeline": "持续运行",
            },
        }
            ...
            gaps.append(
                {
                    ...
                    "gap": self.target_config["survival_threshold"]
                    - production["avg_p_star"],
                    "severity": "CRITICAL",
                }
            )
            ...
            gaps.append(
                {
                    ...
                    "gap": self.target_config["target_coverage"][0]
                    - production["coverage_at_78"],
                    "severity": "HIGH",
                }
            )
    ...
    def _create_alignment_plan(self, production: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "implementation": (
                "立即执行" if plan_type != "MODEL_RETRAIN" else "需要重新训练"
            ),
        }
    ...
    async def execute_alignment_fix(self):
        ...
        report = {
            ...
            "fix_plan": plan,
            "sql_files": [
                f"fix_production_{i}.sql" for i in range(1, len(fix_sqls) + 1)
            ],
        }
