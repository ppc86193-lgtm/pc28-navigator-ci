...
class PC28IntegratedSystem:
    ...
    async def check_upstream_api(self) -> Dict[str, Any]:
        ...
        sign_string = f"appid{self.upstream_appid}formatjsontime{timestamp}{self.upstream_secret}"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.upstream_base}/259", params=params, timeout=10) as response:
        ...
        try:
            ...
            if results:
                ...
                return {
                    ...
                    "avg_vote_ratio": float(row.avg_vote_ratio) if row.avg_vote_ratio else 0,
    ...
    def _recommend_threshold(self, data) -> Dict[str, Any]:
        ...
        if avg_p_star >= self.survival_threshold:
            ...
            recommended_threshold = max(self.survival_threshold + 0.05, avg_p_star - 0.1)
            reasoning = f"平均p_star({avg_p_star:.3f})高于生存线，建议降低阈值提高覆盖率"
        else:
            # 如果平均p_star低于生存线，需要提高模型质量
            recommended_threshold = self.threshold_conf_raw
            reasoning = f"平均p_star({avg_p_star:.3f})低于生存线，需要优化模型而非降低阈值"
        
        return {
            "current_threshold": self.threshold_conf_raw,
            "recommended_threshold": round(recommended_threshold, 3),
            "reasoning": reasoning,
            "risk_assessment": "低风险" if recommended_threshold >= self.survival_threshold else "高风险"
        }
    ...
    def _analyze_breakdown_point(self, checks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity": "CRITICAL" if len(failed_stages) > 2 else "HIGH" if failed_stages else "MEDIUM"
        }
    ...
    async def run_integrated_diagnosis(self):
        ...
        print(f"   工作正常: {len(chain_analysis['breakdown_analysis']['working_stages'])}个环节")
        print(f"   故障环节: {len(chain_analysis['breakdown_analysis']['failed_stages'])}个")
        ...
        integrated_report = {
            ...
            "integration_summary": self._generate_integration_summary(upstream_result, performance, chain_analysis)
        }
        
        # 保存报告
        report_file = f"integrated_diagnosis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_integration_summary(self, upstream: Dict, performance: Dict, chain: Dict) -> Dict[str, Any]:
        """生成整合总结"""
        summary = {
        ...
        if "error" not in performance:
            ...
            if avg_p_star < self.survival_threshold:
                summary["overall_health"] = "CRITICAL"
                summary["critical_issues"].append(f"平均p_star({avg_p_star:.3f})低于生存线")
                summary["immediate_actions"].append("优化模型或调整策略")
            
            coverage_78 = performance.get("coverage_analysis", {}).get("at_78_threshold", 0)
            if coverage_78 == 0:
        ...
        if chain["breakdown_analysis"]["severity"] in ["CRITICAL", "HIGH"]:
            summary["overall_health"] = "CRITICAL"
            summary["critical_issues"].append(chain["breakdown_analysis"]["breakdown_point"])
