...
class NavigatorFullDeployment:
    ...
    def __init__(self):
        ...
        print(f"🚀 Navigator全面部署执行器初始化")
        print(f"🤖 可用AI模型: {len(self.ai_models)}个")
        print(f"💰 资源: AI/ML API不限量 + Google Cloud $300")
    
    async def _make_api_request(self, session, model_id, payload, max_retries=3, base_delay=1):
        """通用API请求辅助函数，带重试逻辑"""
        headers = {
            ...
            try:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        print(f"      ⚠️ 服务端错误 HTTP {response.status} for {model_id}, retrying... ({attempt + 1}/{max_retries})")
                        delay = base_delay * (2 ** attempt)
                        await asyncio.sleep(delay)
                    else:
                        error_text = await response.text()
                        print(f"      ❌ 客户端错误 HTTP {response.status} for {model_id}: {error_text}")
                        return None # Fail fast on non-500 errors
            except Exception as e:
                print(f"      ❌ 请求异常 for {model_id}: {str(e)}, retrying... ({attempt + 1}/{max_retries})")
                delay = base_delay * (2 ** attempt)
                await asyncio.sleep(delay)
        ...
        async with aiohttp.ClientSession() as session:
            for model_id, config in self.ai_models.items():
                ...
                test_payload = {
                    "model": model_id,
                    "messages": [
                        {"role": "system", "content": f"你是PC28 Navigator的{config['role']}Agent"},
        ...
        try:
            ...
            if production_data:
                ...
                analysis_context = {
                    ...
                    "avg_p_star": float(production_data.avg_p_star) if production_data.avg_p_star else 0,
                    "tier_levels": production_data.tier_levels,
                    "prediction_count": int(production_data.prediction_count),
                    "avg_ensemble_p_star": float(production_data.avg_ensemble_p_star) if production_data.avg_ensemble_p_star else 0,
                    "latest_prediction": production_data.latest_prediction.isoformat() if production_data.latest_prediction else None
                }
...
要求: 基于真实数据，提供可执行的具体建议。"""
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "你是PC28 Navigator的首席AI分析师，专门分析生产环境问题"},
        ...
        async with aiohttp.ClientSession() as session:
            data = await self._make_api_request(session, "openai/gpt-5-2025-08-07", payload, max_retries=2, base_delay=5)
    ...
    async def monitor_with_gemini_flash(self):
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "你是PC28实时监控Agent，快速检测系统状态"},
                {"role": "user", "content": f"实时监控检查 - 时间: {datetime.now().isoformat()}"}
            ],
            "max_tokens": 50,
            "temperature": 0.2
        }
        ...
        async with aiohttp.ClientSession() as session:
            data = await self._make_api_request(session, "google/gemini-2.5-flash", payload)
    ...
    async def execute_full_deployment(self):
        ...
        print(f"   GPT-5分析: {'✅ 完成' if gpt5_analysis.get('success') else '❌ 失败'}")
