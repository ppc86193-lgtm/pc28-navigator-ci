    ...
    async def fix_candidates_table_data(self):
        ...
        try:
            ...
            if current_count == 0:
                ...
                self.fixes_completed.append(f"candidates表数据修复: 插入{new_count}行数据")
                
                return {"success": True, "rows_inserted": new_count}
            else:
                print(f"   ℹ️ 表已有数据，无需修复")
...
"""
            ...
            print(f"   ✅ 实时监控脚本已创建: real_time_monitor.py")
...
📊 如果您收到此消息，说明Telegram推送功能已修复！"""
        ...
        if result.get("success"):
            ...
            self.fixes_completed.append(f"Telegram推送功能修复成功")
    ...
    async def generate_fix_report(self, candidates_fix, kpi_fix, monitoring_fix, telegram_fix, monitor_start):
        """生成修复报告"""
        print(f"\n📋 生成修复报告...")
        ...
        fix_report = {
            ...
            "summary": {
                ...
                "overall_status": "大部分修复成功" if len(self.fixes_completed) > len(self.fixes_failed) else "需要进一步修复"
            }
        }
...
👑 系统已真正修复并运行！"""
    ...
    async def execute_comprehensive_fix(self):
        ...
        # 6. 生成修复报告
        fix_report = await self.generate_fix_report(candidates_fix, kpi_fix, monitoring_fix, telegram_fix, monitor_start)
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(comprehensive_fix_report, f, indent=2, ensure_ascii=False, default=str)
