...
class PC28CloudBuildDeployer:
    ...
    async def create_cloudbuild_config(self):
            ...
            "steps": [
                {
                    ...
                    "args": [
                        "build",
                        "-t", f"gcr.io/{self.project_id}/pc28-training-agent:latest",
                ...
                },
                {
                    ...
                    "args": [
                        "run", "deploy", "pc28-training-agent",
                        "--image", f"gcr.io/{self.project_id}/pc28-training-agent:latest",
                        "--platform", "managed",
        ...
        print(f"   ✅ cloudbuild.yaml 已创建")
        print(f"   🏗️ 构建步骤: 3步 (构建->推送->部署)")
        print(f"   ⏱️ 超时设置: 20分钟")
        ...
        for api in required_apis:
            try:
                ...
                if result.returncode == 0:
                    ...
                    enabled_apis.append({"api": api, "status": "FAILED", "error": result.stderr})
                    
            except Exception as e:
                print(f"      ❌ {api} 启用异常: {e}")
                enabled_apis.append({"api": api, "status": "ERROR", "error": str(e)})
        
        print(f"   📊 API启用完成: {len([a for a in enabled_apis if a['status'] == 'ENABLED'])}/{len(required_apis)}")
    ...
    async def deploy_bigquery_functions(self):
        ...
        print(f"   📊 存储过程部署完成: {len([p for p in deployed_procedures if p['status'] == 'DEPLOYED'])}/{len(procedures)}")
        
        return {
            "procedures": deployed_procedures,
            "success_count": len([p for p in deployed_procedures if p['status'] == 'DEPLOYED'])
        }
    ...
    async def execute_cloud_deployment(self):
        ...
        print(f"   API启用: {api_result['success_count']}/{len(api_result['required_apis'])}")
        print(f"   构建提交: {build_result['status']}")
        print(f"   BigQuery: {bq_result['success_count']}个存储过程")
        print(f"   📄 部署报告: {report_file}")
        
        print(f"\n👑 向项目总指挥大人汇报:")
        print(f"   🏗️ Cloud Build已启动！")
        print(f"   ☁️ 完全在云端构建部署！")
        print(f"   💰 使用$300免费额度！")
        print(f"   🤖 Agent们即将真正干活！")
