                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if bot_info.get("ok"):
                            ...
                            return {"success": True, "bot_info": result}
                        else:
                    ...
                    predictions.append(
                        {
                            "issue": row.issue,
                            "p_star_ens": (
                                float(row.p_star_ens) if row.p_star_ens else 0
                            ),
                            "size_pred": row.size_pred,
                            "odd_even_pred": row.odd_even_pred,
                            "confidence": (
                                float(row.confidence) if row.confidence else 0
                            ),
                            "timestamp": (
        ...
        try:
            ...
            if results:
                ...
                kpi_data = {
                    ...
                    "expected_value": (
                        float(kpi.expected_value) if kpi.expected_value else 0
                    ),
                    "coverage_rate": (
                        float(kpi.coverage_rate) if kpi.coverage_rate else 0
                    ),
                    "total_predictions": (
                        int(kpi.total_predictions) if kpi.total_predictions else 0
                    ),
                    "correct_predictions": (
                        int(kpi.correct_predictions) if kpi.correct_predictions else 0
                    ),
                }
...
{'🏆 表现优秀！' if status == '🟢 优秀' else '💪 继续优化！'}"""
    ...
    async def execute_telegram_integration(self):
        ...
        print(
            f"   命令设置: {'✅ 完成' if commands_result.get('success') else '❌ 失败'}"
        )
        print(
            f"   数据连接: {'✅ 就绪' if predictions_result.get('success') else '⚠️ 待配置'}"
        )
